vukalovich_data = table2array(readtable('./Hgproperties/Vukalovich_Saturation_Line_by_Temperature_ASCII.csv'));

% NA is NASG with p_inf = 0 by construction. Parameters from ideal gas approx (taken from NASG; Pg. 9 of LeMétayer-Saurel, below Eq. 45 and 46).
b_gas_all = 0;
...
% NA gas phase is just ideal gas. % Using Eq. 50, 51, 55 in LeMétayer-Saurel to calculate c_p,g; q_g; c_v,g; gamma_g for both full data set and range. c_p,g in J/(g K); c_v,g in m^3/kg.
v_gas_analytic = (ideal_gas_const .* vukalovich_data(:,1)) ./ vukalovich_data(:,2);
...
% c_1 coefficients
c_p_liq_numer_1st_term_all = sum(vukalovich_data(:,1) .* (vukalovich_data(:,7) - h_liq_avg_all)); %c_1 (all)

% c_2 coefficients
c_p_liq_numer_2nd_term_coeff_all = sum(vukalovich_data(:,1) .* (vukalovich_data(:,2) - p_avg_all)); %c_2 (all)
...
for i=1:88
  f1_all = f1_all + (vpa(vukalovich_data(i,1)) * (vpa(vukalovich_data(i,4)) - v_liq_avg_all)/(vpa(vukalovich_data(i,2)) + p_inf_liq));
  f2_all = f2_all + (1/88) * (vpa(vukalovich_data(i,1))/(vpa(vukalovich_data(i,2)) + p_inf_liq));
  f3_all = f3_all + vpa(vukalovich_data(i,1))/(vpa(vukalovich_data(i,2)) + p_inf_liq) * (vpa(vukalovich_data(i,1))/(vpa(vukalovich_data(i,2)) + p_inf_liq) - (1/88) * (vpa(vukalovich_data(i,1))/(vpa(vukalovich_data(i,2)) + p_inf_liq)));
end
...
for i=1:88
  compress_factor_gas_analytic(i) = (vukalovich_data(i,2) .* v_gas_analytic(i)) ./ (ideal_gas_const .* vukalovich_data(i,1));
...
figure(1);
  ...
  legend('Experimental values' + string(newline) + '(Vukalovich)','NA gas phase' + string(newline) + 'theoretical prediction','Location','northeast','Interpreter','latex','FontSize',28);
...
figure(2);
  ...
  legend('Experimental values' + string(newline) + '(Vukalovich)','NA gas phase' + string(newline) + 'theoretical prediction','Location','southeast','Interpreter','latex','FontSize',28);
  set(gca,'TickLabelInterpreter','latex');
...
figure(5);
  ...
  legend('Experimental values' + string(newline) + '(Vukalovich)','NA gas phase' + string(newline) + 'theoretical prediction','Location','southwest','Interpreter','latex','FontSize',28);
  set(gca,'TickLabelInterpreter','latex');

%% NA is NASG with p_inf_liq = 0
LeM_Eq_64_T_P_frac_all = vukalovich_data(:,1) ./ vukalovich_data(:,2);
LeM_Eq_64_num_all = sum(LeM_Eq_64_T_P_frac_all .* (vukalovich_data(:,4) - v_liq_avg_all));
...
v_liq_analytic_all = zeros(88,1);
h_liq_analytic_all = zeros(88,1);
dens_liq_analytic_all = zeros(88,1);
dens_liq_vukalovich = zeros(88,1);

c_p_liq_all = (sum(vukalovich_data(:,1) .* (vukalovich_data(:,7) - h_liq_avg_all)) - (b_liq_all .* sum(vukalovich_data(:,1) .* (vukalovich_data(:,2) - p_avg_all))))/sum(vukalovich_data(:,1) .* (vukalovich_data(:,1) - T_avg_all));
c_v_liq_all = c_p_liq_all - c_p_liq_minus_c_v_liq_all;
q_liq_all = h_liq_avg_all - c_p_liq_all .* T_avg_all - b_liq_all .* p_avg_all;

for i=1:88
  v_liq_analytic_all(i) = ((c_p_liq_minus_c_v_liq_all * vukalovich_data(i,1)) ./ vukalovich_data(i,2)) + b_liq_all;
  dens_liq_analytic_all(i) = 1./v_liq_analytic_all(i);
  h_liq_analytic_all(i) = (c_p_liq_all .* vukalovich_data(i,1)) + (b_liq_all .* vukalovich_data(i,2)) + q_liq_all;
...
figure(6);
  ...
  legend('Experimental values' + string(newline) + '(Vukalovich)','NA liquid phase' + string(newline) + 'theoretical prediction','Location','northwest','Interpreter','latex','FontSize',28);
  set(gca,'TickLabelInterpreter','latex');

figure(7);
  semilogy(vukalovich_data(:,1),dens_liq_vukalovich,'.','Color',[0.3010 0.7450 0.9330],'MarkerSize',16);
  hold on;
  semilogy(vukalovich_data(:,1),dens_liq_analytic_all,'-','Color',[0.8500 0.3250 0.0980],'LineWidth',1.5);
  fontsize(30,"points");
  %title('\bf{Specific Volume vs. Temperature, Mercury Gas, NA}','Interpreter','latex','FontSize', 40);
  xlabel('$T \left( \mathrm{K} \right)$','Interpreter','latex');
  ylabel('${\rho}_{l} \left( \mathrm{kg / {m}^{3}} \right)$','Interpreter','latex');
  legend('Experimental values' + string(newline) + '(Vukalovich)','NA liquid phase' + string(newline) + 'theoretical prediction','Location','northeast','Interpreter','latex','FontSize',28);
  set(gca,'TickLabelInterpreter','latex');
...
figure(8);
  ...
  legend('Experimental values' + string(newline) + '(Vukalovich)','NA liquid phase' + string(newline) + 'theoretical prediction','Location','southeast','Interpreter','latex','FontSize',28);
  set(gca,'TickLabelInterpreter','latex');
...
q_prime_gas_all = (c_p_gas_all - c_p_liq_all) + (c_p_gas_all - c_v_gas_all) .* mean(log(vukalovich_data(:,2)) - (LeM_Eq_41_B_coeff_all + (LeM_Eq_41_E_coeff_all .* vukalovich_data(:,2)))./vukalovich_data(:,1) - LeM_Eq_41_C_coeff_all .* log(vukalovich_data(:,2)) - LeM_Eq_41_D_coeff_all .* log((vukalovich_data(:,2))));
q_prime_liq_all = 0;
...
for i=1:88
  s_liq_analytic_all = c_v_liq_all .* log(((vukalovich_data(:,1) .^ gamma_liq_all) ./ ((vukalovich_data(:,2)) .^(gamma_liq_all - 1)))) + q_prime_liq_all;
  s_gas_analytic_all = c_v_gas_all .* log(((vukalovich_data(:,1) .^ gamma_gas_all) ./ ((vukalovich_data(:,2)) .^(gamma_gas_all - 1)))) + q_prime_gas_all;
  cs_sq_liq_analytic_all = (gamma_liq_all .* (vukalovich_data(:,4) .^ 2) .* (vukalovich_data(:,2))) ./ (vukalovich_data(:,4) - b_liq_all);
end
...
%%
figure(4);
  plot(vukalovich_data(:,1),vukalovich_data(:,11),'.','Color',[0.4390 0.1650 0.5290],'MarkerSize',16);
  hold on;
  plot(vukalovich_data(:,1),s_gas_analytic_all,'-','Color',[0.6350 0.0780 0.1840],'LineWidth',1.5);
  fontsize(30,"points");
...
figure(9);
  plot(vukalovich_data(:,1),vukalovich_data(:,10),'.','Color',[0.4390 0.1650 0.5290],'MarkerSize',16);
  hold on;
  plot(vukalovich_data(:,1),s_liq_analytic_all,'-','Color',[0.6350 0.0780 0.1840],'LineWidth',1.5);
  fontsize(30,"points");
  %title('Specific Entropy vs. Temperature, Mercury Gas, NA');
  xlabel('$T \left( \mathrm{K} \right)$','Interpreter','latex');
