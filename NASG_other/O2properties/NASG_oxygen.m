oxygen_data = table2array(readtable('./../O2properties/Oxygen_NIST_CSV_Vukalovich_order_ASCII'));
...
T_avg_all = mean(oxygen_data(:,1));
p_avg_all = mean(oxygen_data(:,2));
h_gas_avg_all = mean(oxygen_data(:,8));
h_liq_avg_all = mean(oxygen_data(:,7));
v_liq_avg_all = mean(oxygen_data(:,4));

%c_3 constants
T_avg_norm_factor_all = sum(oxygen_data(:,1) .* (oxygen_data(:,1) - T_avg_all));
...
c_p_gas_all = sum(oxygen_data(:,1) .* (oxygen_data(:,8) - h_gas_avg_all))/T_avg_norm_factor_all;
q_gas_all = h_gas_avg_all - c_p_gas_all .* T_avg_all;
c_v_gas_all = c_p_gas_all - sum(oxygen_data(:,1) .* oxygen_data(:,5) ./ oxygen_data(:,2))/sum((oxygen_data(:,1) ./ oxygen_data(:,2)) .^ 2);
...
reference_p = 0.9935;
reference_dens = 1142.1;
reference_sound_speed_sq = (905.9).^2;
...
% c_1 coefficients
c_p_liq_numer_1st_term_all = sum(oxygen_data(:,1) .* (oxygen_data(:,7) - h_liq_avg_all)); %c_1 (all)

% c_2 coefficients
c_p_liq_numer_2nd_term_coeff_all = sum(oxygen_data(:,1) .* (oxygen_data(:,2) - p_avg_all)); %c_2 (all)
...
for i=1:data_size
  f1_all = f1_all + (vpa(oxygen_data(i,1)) * (vpa(oxygen_data(i,4)) - v_liq_avg_all)/(vpa(oxygen_data(i,2)) + p_inf_liq));
  f2_all = f2_all + (1/data_size) * (vpa(oxygen_data(i,1))/(vpa(oxygen_data(i,2)) + p_inf_liq));
  f3_all = f3_all + vpa(oxygen_data(i,1))/(vpa(oxygen_data(i,2)) + p_inf_liq) * (vpa(oxygen_data(i,1))/(vpa(oxygen_data(i,2)) + p_inf_liq) - (1/data_size) * (vpa(oxygen_data(i,1))/(vpa(oxygen_data(i,2)) + p_inf_liq)));
end
...
for i=1:data_size
  v_gas_analytic(i) = ((gamma_gas_all - 1) * c_v_gas_all * oxygen_data(i,1))/(oxygen_data(i,2));
  compress_factor_gas_analytic(i) = (oxygen_data(i,2) .* v_gas_analytic(i)) ./ (ideal_gas_const .* oxygen_data(i,1) .* 4.9856360761414);
  h_gas_analytic(i) = gamma_gas_all * c_v_gas_all * oxygen_data(i,1) + q_gas_all;
  dens_gas_analytic(i) = 1 ./ v_gas_analytic(i);
  dens_gas_vukalovich(i) = 1 ./ oxygen_data(i,5);
end
...
%%
figure(1);
  semilogy(oxygen_data(:,1),oxygen_data(:,5),'.','Color',[0 0.4470 0.7410],'MarkerSize',16);
  hold on;
  semilogy(oxygen_data(:,1),v_gas_analytic,'-','Color',[0.8500 0.3250 0.0980],'LineWidth',1.5);
  fontsize(15,"points");
...
figure(2);
  semilogy(oxygen_data(:,1),dens_gas_vukalovich,'.','Color',[0.3010 0.7450 0.9330],'MarkerSize',16);
  hold on;
  semilogy(oxygen_data(:,1),dens_gas_analytic,'-','Color',[0.8500 0.3250 0.0980],'LineWidth',1.5);
  fontsize(15,"points");
  %title('\bf{Specific Volume vs. Temperature, Mercury Gas, NA}','Interpreter','latex','FontSize', 40);
  xlabel('$T \left( \mathrm{K} \right)$','Interpreter','latex');
...
figure(3);
  plot(oxygen_data(:,1),oxygen_data(:,8),'.','Color',[0.4940 0.1840 0.5560],'MarkerSize',16);
  hold on;
  plot(oxygen_data(:,1),h_gas_analytic,'-','Color',[0.9290 0.6940 0.1250],'LineWidth',1.5);
  fontsize(15,"points");
...
figure(5);
  plot(oxygen_data(:,1),oxygen_data(:,6),'.','Color',[0.7 0 1],'MarkerSize',16);
  hold on;
  plot(oxygen_data(:,1),compress_factor_gas_analytic,'-','Color',[0.9290 0.6940 0.1250],'LineWidth',1.5);
  fontsize(15,"points");
...
LeM_Eq_64_T_P_frac_all = oxygen_data(:,1) ./ (oxygen_data(:,2) + p_inf_liq_result_all);
LeM_Eq_64_num_all = sum(LeM_Eq_64_T_P_frac_all .* (oxygen_data(:,4) - v_liq_avg_all));
...
for i=1:data_size_full
  %v_liq_analytic_all(i) = ((gamma_liq_all - 1) .* c_p_liq_minus_c_v_liq_all .* oxygen_data(i,1)) ./ (oxygen_data(i,2) + p_inf_liq_result_all) + b_liq_all;
  %v_liq_analytic_all(i) = (((c_p_liq_all - c_v_liq_all) .* oxygen_data(i,1))./(oxygen_data(i,2) + p_inf_liq_result_all)) + b_liq_all;
  v_liq_analytic_all(i) = (((c_p_liq_minus_c_v_liq_all) .* oxygen_data(i,1))./(oxygen_data(i,2) + p_inf_liq_result_all)) + b_liq_all;
  dens_liq_analytic_all(i) = 1./v_liq_analytic_all(i);
  dens_liq_vukalovich(i) = 1./oxygen_data(i,4);
end
...
%%
figure(6);
  plot(oxygen_data(:,1),oxygen_data(:,4),'.','Color',[0.4660 0.6740 0.1880],'MarkerSize',16);
  hold on;
  plot(oxygen_data(:,1),v_liq_analytic_all,'-','Color',[0.9290 0.6940 0.1250],'LineWidth',1.5);
  fontsize(30,"points");
...
figure(7);
  semilogy(oxygen_data(:,1),dens_liq_vukalovich,'.','Color',[0.3010 0.7450 0.9330],'MarkerSize',16);
  hold on;
  semilogy(oxygen_data(:,1),dens_liq_analytic_all,'-','Color',[0.8500 0.3250 0.0980],'LineWidth',1.5);
  fontsize(30,"points");
  %title('\bf{Specific Volume vs. Temperature, Mercury Gas, NA}','Interpreter','latex','FontSize', 40);
  xlabel('$T \left( \mathrm{K} \right)$','Interpreter','latex');
  ...
  set(gca,'TickLabelInterpreter','latex');
%%
c_p_liq_all = (sum(oxygen_data(:,1) .* (oxygen_data(:,7) - h_liq_avg_all)) - (b_liq_all .* sum(oxygen_data(:,1) .* (oxygen_data(:,2) - p_avg_all))))/sum(oxygen_data(:,1) .* (oxygen_data(:,1) - T_avg_all));
...
for i=1:data_size_full
  h_liq_analytic_all(i) = (c_p_liq_all .* oxygen_data(i,1)) + (b_liq_all .* oxygen_data(i,2)) + q_liq_all;
end
...
%%
figure(8);
  plot(oxygen_data(:,1),oxygen_data(:,7),'.','Color',[0.6350 0.0780 0.1840],'MarkerSize',16);
  hold on;
  plot(oxygen_data(:,1),h_liq_analytic_all,'-','Color',[0.9290 0.6940 0.1250],'LineWidth',1.5);
  fontsize(30,"points");
...
q_prime_gas_all = (c_p_gas_all - c_p_liq_all) + (c_p_gas_all - c_v_gas_all) .* mean(log(oxygen_data(:,2)) - (LeM_Eq_41_B_coeff_all + (LeM_Eq_41_E_coeff_all .* water_data_full(:,2)))./water_data_full(:,1) - LeM_Eq_41_C_coeff_all .* log(water_data_full(:,2)) - LeM_Eq_41_D_coeff_all .* log((water_data_full(:,2) + p_inf_liq_result_all)));
q_prime_liq_all = 0;
...
for i=1:data_size
  s_liq_analytic_all = c_v_liq_all .* log(((oxygen_data(:,1) .^ gamma_liq_all) ./ ((oxygen_data(:,2) + p_inf_liq_result_all) .^(gamma_liq_all - 1)))) + q_prime_liq_all;
  s_gas_analytic_all = c_v_gas_all .* log(((oxygen_data(:,1) .^ gamma_gas_all) ./ ((oxygen_data(:,2) + p_inf_gas_all) .^(gamma_gas_all - 1)))) + q_prime_gas_all;
  cs_sq_liq_analytic_all = (gamma_liq_all .* (oxygen_data(:,4) .^ 2) .* (oxygen_data(:,2) - p_inf_liq_result_all)) ./ (oxygen_data(:,4) - b_liq_all);
end
...
%%
figure(4);
  plot(oxygen_data(:,1),oxygen_data(:,11),'.','Color',[0.4390 0.1650 0.5290],'MarkerSize',16);
  hold on;
  plot(oxygen_data(:,1),s_gas_analytic_all,'-','Color',[0.6350 0.0780 0.1840],'LineWidth',1.5);
  fontsize(30,"points");
...
figure(9);
  plot(oxygen_data(:,1),oxygen_data(:,10),'.','Color',[0.4390 0.1650 0.5290],'MarkerSize',16);
  hold on;
  plot(oxygen_data(:,1),s_liq_analytic_all,'-','Color',[0.6350 0.0780 0.1840],'LineWidth',1.5);
  fontsize(30,"points");
  %title('Specific Entropy vs. Temperature, Mercury Gas, NA');
  xlabel('$T \left( \mathrm{K} \right)$','Interpreter','latex');
...
for i=1:data_size_full
  v_liq_analytic_first_way(i) = ((gamma_liq_all - 1) .* c_p_liq_minus_c_v_liq_all .* oxygen_data(i,1)) ./ (oxygen_data(i,2) + p_inf_liq_result_all) + b_liq_all;
  v_liq_analytic_second_way(i) = (((c_p_liq_all - c_v_liq_all) .* oxygen_data(i,1))./(oxygen_data(i,2) + p_inf_liq_result_all)) + b_liq_all;
  v_liq_analytic_third_way(i) = (((c_p_liq_minus_c_v_liq_all) .* oxygen_data(i,1))./(oxygen_data(i,2) + p_inf_liq_result_all)) + b_liq_all;
end

%%
figure(10);
  semilogy(oxygen_data(:,1),oxygen_data(:,4),'.','Color',[0 0.4470 0.7410],'MarkerSize',16);
  hold on;
  semilogy(oxygen_data(:,1),v_liq_analytic_first_way,'-','Color',[0.8500 0.3250 0.0980],'LineWidth',1.5);
  hold on;
  semilogy(oxygen_data(:,1),v_liq_analytic_second_way,'-','Color',[0.4390 0.1650 0.5290],'LineWidth',1.5);
  hold on;
  semilogy(oxygen_data(:,1),v_liq_analytic_third_way,'-','Color',[0.6350 0.0780 0.1840],'LineWidth',1.5);
  fontsize(15,"points");
  %title('\bf{Specific Volume vs. Temperature, Mercury Gas, NA}','Interpreter','latex','FontSize', 40);
  xlabel('$T \left( \mathrm{K} \right)$','Interpreter','latex');
  ylabel('${v}_{g} \left( \mathrm{{m}^{3} / kg} \right)$','Interpreter','latex');
